...
//        });
        ...
        userbean = (User) ACache.get(SearchAct.this).getAsObject("userbean");
        sc_rv.setLayoutManager(new LinearLayoutManager(SearchAct.this));
        adapter = new SimpleAdapter<>(R.layout.item_circle, list, new SimpleAdapter.ConVert<CircleBean>() {
            ...
            public void convert(BaseViewHolder helper, CircleBean circleBean) {
                ...
                if (!TextUtils.isEmpty(circleBean.head)){
                    Glide.with(SearchAct.this).load(circleBean.head).into(view);
                }
                ...
                if (!TextUtils.isEmpty(circleBean.imgs)){
                    ...
                    recyclerView.setLayoutManager(new GridLayoutManager(SearchAct.this,3));
                    SimpleAdapter    aaa = new SimpleAdapter<>(R.layout.item_photo, pahtList, new SimpleAdapter.ConVert<String>() {
                        ...
                        public void convert(BaseViewHolder helper, final String path) {
                            ...
                            Glide.with(SearchAct.this).load(path).into(view);
                        }
                    });
    ...
    private void getPlNum(String c_id, final TextView  textView) {
        ...
        bmobQuery.findObjects(SearchAct.this, new FindListener<PingLun>() {
            @Override
    ...
    private void getZnNum(String c_id, final TextView  textView) {
        ...
        bmobQuery.addWhereEqualTo("c_id",c_id);
        bmobQuery.findObjects(SearchAct.this, new FindListener<Zan>() {
            @Override
...
//                for (int i=0;i<list.size();i++){ if(list.get(i).getFoodName().contains(mEtName.getText().toString
                ...
                bmobQuery.findObjects(SearchAct.this, new FindListener<CircleBean>() {
                    @Override
                    public void onSuccess(List<CircleBean> list2) {
//                        search_swipe.setRefreshing(false);
                        if(list2.size()>0){
//                            Toast.makeText(SearchAct.this, "目前里面有数据", Toast.LENGTH_SHORT).show();
                        }else {
                            Toast.makeText(SearchAct.this, "目前里面没有数据", Toast.LENGTH_SHORT).show();
                        ...
                        for (int i = 0; i < list2.size(); i++) {//拉黑的不显示
                            ...
                            bmobQuery1.findObjects(SearchAct.this, new FindListener<LaHei>() {
                                @Override
...
//                                        search_swipe.setVisibility(View.GONE);
                    ...
                    @Override
                    public void onError(int i, String s) {
                        Toast.makeText(SearchAct.this, "服务器异常稍后再试", Toast.LENGTH_SHORT).show();
                    }
